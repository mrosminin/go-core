...
import (
	"fmt"
	"go-core-own/homework-15/pkg/engine"
	"go-core-own/homework-15/pkg/index"
	"go-core-own/homework-15/pkg/scanner"
	"go-core-own/homework-15/pkg/scanner/spider"
	"go-core-own/homework-15/pkg/storage"
	"go-core-own/homework-15/pkg/storage/btree"
	"go-core-own/homework-15/pkg/storage/diskstor"
	"go-core-own/homework-15/pkg/webapp"
	"log"
	"sync"
)
...
func new() (*gosearch, error) {
	...
	// Веб-приложение
	gs.wa = webapp.New(gs.index, gs.storage)
...
func main() {
	...
	go func() {
		err = gs.wa.Init()
		if err != nil {
			fmt.Printf("\nОшибка при запуске веб-сервера: %v", err)
		}
	}()
